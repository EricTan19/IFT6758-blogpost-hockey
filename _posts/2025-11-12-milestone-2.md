...
Across the regular season test set, overall performance is consistent with what was seen during validation, with XGBoost and the MLP again emerging as the strongest models. The ROC curves show that XGBoost achieves the highest AUC (0.756), closely followed by the MLP (0.755), indicating good ranking ability. Logistic Regression with both distance and angle performs better than logistic models using a single feature, confirming the importance of combining spatial information. The goal-rate and cumulative-goals curves show that the better models consistently assign higher predicted probabilities to shots that become goals, suggesting good probability ordering. Calibration plots indicate that all models slightly under-predict goal likelihood at higher probabilities, although XGBoost shows some overconfidence near the top bins. Overall, the models generalize well, and no major drop in performance is observed compared with validation. Overall, the models generalize well to the regular season test set, and none show unexpected performance drops.

On the playoff test set, model rankings remain similar, though overall AUC scores are slightly lower, suggesting that playoff gameplay is harder to model. XGBoost still leads (AUC 0.756), with the MLP close behind, while single-feature logistic models show noticeably weaker separation. Goal-rate curves reveal more variability across percentiles, likely due to smaller sample sizes in playoff games. Cumulative-goal plots show that XGBoost and MLP again capture high-probability goals earlier, but with more noise than in the regular season. Calibration curves also become less stable—particularly at high predicted probabilities—indicating reduced reliability on playoff data. Overall, the models generalize reasonably well, but playoff performance is slightly worse and less stable than during the regular season. In conclusion, the models maintain reasonable generalization to playoff data, but increased variability and weaker calibration indicate that playoff scenarios are harder to model reliably than the regular season.

![Screenshot](milestone2_part_7_roc.png)

![Screenshot](milestone2_part_7_goal_rate.png)

![Screenshot](milestone2_part_7_cumulative.png)

![Screenshot](milestone2_part_7_calibration_curve.png)
